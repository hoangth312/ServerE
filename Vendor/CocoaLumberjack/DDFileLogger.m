...
@interface DDsLogFileManagerDefault (PrivateAPI)
...
@implementation DDsLogFileManagerDefault
                       ...
                       context:(void *)context
{
	...
	if ([keyPath isEqualToString:@"maximumNumberOfLogFiles"])
	{
		...
		dispatch_async([DDsLog loggingQueue], ^{ @autoreleasepool {
...
- (void)deleteOldLogFiles
{
	NSLogVerbose(@"DDsLogFileManagerDefault: deleteOldLogFiles");
	...
	if (count > 0)
	{
		DDsLogFileInfo *logFileInfo = [sortedLogFileInfos objectAtIndex:0];
	...
	{
		DDsLogFileInfo *logFileInfo = [sortedArchivedLogFileInfos objectAtIndex:i];
		
		NSLogInfo(@"DDsLogFileManagerDefault: Deleting file: %@", logFileInfo.fileName);
...
/**
 * Returns an array of DDsLogFileInfo objects,
...
- (NSArray *)unsortedLogFileInfos
{
	...
	for (NSString *filePath in unsortedLogFilePaths)
	{
		DDsLogFileInfo *logFileInfo = [[DDsLogFileInfo alloc] initWithFilePath:filePath];
...
- (NSArray *)sortedLogFilePaths
{
	...
	for (DDsLogFileInfo *logFileInfo in sortedLogFileInfos)
	{
...
- (NSString *)createNewLogFile
{
	...
	do
	{
		...
		{
			NSLogVerbose(@"DDsLogFileManagerDefault: Creating new log file: %@", fileName);
...
@implementation DDsLogFileFormatterDefault
...
- (id)init
{
	DDsLogFileManagerDefault *defaultLogFileManager = [[DDsLogFileManagerDefault alloc] init];
	
	return [self initWithLogFileManager:defaultLogFileManager];
}

- (id)initWithLogFileManager:(id <DDsLogFileManager>)aLogFileManager
{
	if ((self = [super init]))
	{
		...
		formatter = [[DDsLogFileFormatterDefault alloc] init];
	}
	return self;
}
...
**/
- (DDsLogFileInfo *)currentLogFileInfo
{
	if (currentLogFileInfo == nil)
	{
		...
		{
			DDsLogFileInfo *mostRecentLogFileInfo = [sortedLogFileInfos objectAtIndex:0];
		...
		if (currentLogFileInfo == nil)
		{
			...
			currentLogFileInfo = [[DDsLogFileInfo alloc] initWithFilePath:currentLogFilePath];
		}
	}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark DDsLogger Protocol
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
{
	return [[DDsLogFileInfo alloc] initWithFilePath:aFilePath];
}
...
- (NSDate *)creationDate
{
	if (creationDate == nil)
	{
		...
		{
			NSLogError(@"DDsLogFileInfo: creationDate(%@): getattrlist result = %i", self.fileName, result);
		}
...
- (void)renameFile:(NSString *)newFileName
{
	...
	if (![newFileName isEqualToString:[self fileName]])
	{
		...
		NSLogVerbose(@"DDsLogFileInfo: Renaming file: '%@' -> '%@'", self.fileName, newFileName);
		
		NSError *error = nil;
		if (![[NSFileManager defaultManager] moveItemAtPath:filePath toPath:newFilePath error:&error])
		{
			NSLogError(@"DDsLogFileInfo: Error renaming file (%@): %@", self.fileName, error);
		}
...
- (void)addExtendedAttributeWithName:(NSString *)attrName
{
	...
	if (result < 0)
	{
		NSLogError(@"DDsLogFileInfo: setxattr(%@, %@): error = %i", attrName, self.fileName, result);
	}
}
...
- (void)removeExtendedAttributeWithName:(NSString *)attrName
{
	...
	{
		NSLogError(@"DDsLogFileInfo: removexattr(%@, %@): error = %i", attrName, self.fileName, result);
	}
}
...
- (BOOL)isEqual:(id)object
{
	...
	{
		DDsLogFileInfo *another = (DDsLogFileInfo *)object;
...
- (NSComparisonResult)reverseCompareByCreationDate:(DDsLogFileInfo *)another
{
...
- (NSComparisonResult)reverseCompareByModificationDate:(DDsLogFileInfo *)another
{
