 ...
 * static const int DDsLogLevel = LOG_LEVEL_VERBOSE;
 * 
 * Step 3:
 * Replace your NSLog statements with DDsLog statements according to the severity of the message.
 * 
 * NSLog(@"Fatal error, no dohickey found!"); -> DDsLogError(@"Fatal error, no dohickey found!");
 * 
 * DDsLog works exactly the same as NSLog.
 ...
 * 
 * if (logFlagForThisLogMsg & DDsLogLevel) { execute log message }
 * 
 ...
 * 
 * (If the compiler sees DDsLogLevel declared as a constant, the compiler simply checks to see if the 'if' statement
 ...
 * 
 * The DDsLogMessage object includes a 'tag' ivar that may be used for a variety of purposes.
 ...
 * 
 * static const int DDsLogLevel = LOG_FLAG_ERROR | LOG_FLAG_INFO;
 * 
 * Flags may also be consulted when writing custom log formatters,
 * as the DDsLogMessage class captures the individual flag that caused the log message to fire.
 * 
 * -- Levels --
 * 
 ...
 *         DDsLogVerbose(@"sprocket: %@", [sprocket description])
 * }
 * 
 * -- Async --
 * 
...
#define LOG_ERROR   (DDsLogLevel & LOG_FLAG_ERROR)
#define LOG_WARN    (DDsLogLevel & LOG_FLAG_WARN)
#define LOG_INFO    (DDsLogLevel & LOG_FLAG_INFO)
#define LOG_VERBOSE (DDsLogLevel & LOG_FLAG_VERBOSE)
...
#define DDsLogError(frmt, ...)   LOG_OBJC_MAYBE(LOG_ASYNC_ERROR,   DDsLogLevel, LOG_FLAG_ERROR,   0, frmt, ##__VA_ARGS__)
#define DDsLogWarn(frmt, ...)    LOG_OBJC_MAYBE(LOG_ASYNC_WARN,    DDsLogLevel, LOG_FLAG_WARN,    0, frmt, ##__VA_ARGS__)
#define DDsLogInfo(frmt, ...)    LOG_OBJC_MAYBE(LOG_ASYNC_INFO,    DDsLogLevel, LOG_FLAG_INFO,    0, frmt, ##__VA_ARGS__)
#define DDsLogVerbose(frmt, ...) LOG_OBJC_MAYBE(LOG_ASYNC_VERBOSE, DDsLogLevel, LOG_FLAG_VERBOSE, 0, frmt, ##__VA_ARGS__)

#define DDsLogCError(frmt, ...)   LOG_C_MAYBE(LOG_ASYNC_ERROR,   DDsLogLevel, LOG_FLAG_ERROR,   0, frmt, ##__VA_ARGS__)
#define DDsLogCWarn(frmt, ...)    LOG_C_MAYBE(LOG_ASYNC_WARN,    DDsLogLevel, LOG_FLAG_WARN,    0, frmt, ##__VA_ARGS__)
#define DDsLogCInfo(frmt, ...)    LOG_C_MAYBE(LOG_ASYNC_INFO,    DDsLogLevel, LOG_FLAG_INFO,    0, frmt, ##__VA_ARGS__)
#define DDsLogCVerbose(frmt, ...) LOG_C_MAYBE(LOG_ASYNC_VERBOSE, DDsLogLevel, LOG_FLAG_VERBOSE, 0, frmt, ##__VA_ARGS__)

/**
 * The THIS_FILE macro gives you an NSString of the file name.
 * For simplicity and clarity, the file name does not include the full path or file extension.
 * 
 * For example: DDsLogWarn(@"%@: Unable to find thingy", THIS_FILE) -> @"MyViewController: Unable to find thingy"
**/
 ...
 * 
 * For example: DDsLogWarn(@"%@ - Requires non-nil strings", THIS_METHOD) -> @"setMake:model: requires non-nil strings"
 * 
...
- (void)setLogFormatter:(id <DDsLogFormatter>)formatter;
 ...
 * 
 * This allows invocations of DDsLog's flushLog method to be propogated to loggers that need it.
 * 
 * Note that DDsLog's flushLog method is invoked automatically when the application quits,
...
@protocol DDsLogFormatter <NSObject>
@required
...
**/
- (NSString *)formatLogMessage:(DDsLogMessage *)logMessage;
...
**/
- (void)didAddToLogger:(id <DDsLogger>)logger;
- (void)willRemoveFromLogger:(id <DDsLogger>)logger;
...
/**
 * The DDsLogMessage class encapsulates information about the log message.
 * If you write custom loggers or formatters, you will be dealing with objects of this class.
**/

enum {
	DDsLogMessageCopyFile     = 1 << 0,
	DDsLogMessageCopyFunction = 1 << 1
};
typedef int DDsLogMessageOptions;

@interface DDsLogMessage : NSObject
{
...
@public
	...
	// For 3rd party extensions that manually create DDsLogMessage instances.
	DDsLogMessageOptions options;
}
 ...
 * Options is a bitmask which supports DDsLogMessageCopyFile and DDsLogMessageCopyFunction.
**/
...
/**
 * The DDsLogger protocol specifies that an optional formatter can be added to a logger.
...
@interface DDAbstractLogger : NSObject <DDsLogger>
{
	id <DDsLogFormatter> formatter;
